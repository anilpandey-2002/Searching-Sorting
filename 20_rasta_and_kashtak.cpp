...
long long s1[1 << 10][1 << 10], s2[1 << 10][1 << 10], ar1[1 << 10][1 << 10], ar2[1 << 10][1 << 10], pw1[1 << 22], pw2[1 << 22], n2, m2, l, r, n1, m1;
vector<long long> have;

long long get1(long a, long b, long c)
{
    return (s1[a + c][b + c] - s1[a][b + c] - s1[a + c][b] + s1[a][b]) * pw1[2000000 - a] * pw2[2000000 - b];
}

long long get2(long a, long b, long c)
{
    return (s2[a + c][b + c] - s2[a][b + c] - s2[a + c][b] + s2[a][b]) * pw1[2000000 - a] * pw2[2000000 - b];
}
...
bool solve(int X)
{
    ...
    for (int i = 0; i + X <= n2; i++)
        for (int j = 0; j + X <= m2; j++)
        {
            ...
            if (binary_search(have.begin(), have.end(), res))
                return true;
        }
    return false;
}
...
int main()
{
    ...
    for (int i = 1; i <= 2000000; i++)
        pw1[i] = pw1[i - 1] * 173, pw2[i] = pw2[i - 1] * 137;
